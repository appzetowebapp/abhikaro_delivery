...
class _WebViewScreenState extends State<WebViewScreen> with WidgetsBindingObserver {
  ...
  Future<void> _showOverlayWindow() async {
    ...
    await FlutterOverlayWindow.showOverlay(
      enableDrag: true,
      overlayTitle: "Abhikaro Delivery Overlay",
      ...
      try {
        if (data is String && data.startsWith('{')) {
          ...
          if (action == 'ACCEPT_ORDER') {
            ...
            // Stop ringtone when accepting from overlay
            ...
            // Stop ringtone when rejecting from overlay
            FlutterBackgroundService().invoke('stopRingtone');
          }
        }
      } catch (e) {
  ...
  Future<void> _toggleOverlay() async {
    ...
    if (_isTrackingEnabled) {
      ...
      if (hasPermission) {
        ...
        // NOW check if location service (GPS) is enabled
        ...
        if (!isServiceEnabled) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: const Text('GPS is off. Please turn it on to start tracking.'),
                backgroundColor: Colors.orange,
                behavior: SnackBarBehavior.floating,
                action: SnackBarAction(
                  label: 'TURN ON',
                    ...
                    InAppWebView(
                      ...
                      onWebViewCreated: (controller) async {
                        ...
                        controller.addJavaScriptHandler(
                          handlerName: 'openCamera',
                          callback: (args) async {
                            debugPrint('📸 openCamera called with args: $args');
                            final ImagePicker picker = ImagePicker();
                            
                            // Determine source
                            ImageSource? source;
                            if (args.isNotEmpty) {
                              final String arg = args[0].toString().toLowerCase();
                              if (arg == 'gallery' || arg.contains('gallery')) {
                                source = ImageSource.gallery;
                              } else if (arg == 'camera' || arg.contains('camera')) {
                                source = ImageSource.camera;
                              }
                            }

                            // If source is still null, show a picker to the user
                            if (source == null) {
                        ...
                        // Add common aliases for image picking
                        for (var name in ['openGallery', 'pickImage', 'selectImage', 'openImagePicker']) {
                          controller.addJavaScriptHandler(
                            handlerName: name,
                            callback: (args) async {
                              debugPrint('📸 Handler $name called with args: $args');
                              // Default source based on handler name
                              ImageSource source = (name == 'openGallery') 
                                  ? ImageSource.gallery 
                                  : ImageSource.gallery; // Default to gallery for generic pickers
                              ...
                              try {
                                ...
                                debugPrint('❌ Error in $name handler: $e');
                              }
                              return {'success': false};
                            },
                          );
                        ...
                        // Add JavaScript handler to stop ringtone
                        controller.addJavaScriptHandler(
                          handlerName: 'stopRingtone',
                          callback: (args) {
                            debugPrint('🔕 JS: stopRingtone called');
                            FlutterBackgroundService().invoke('stopRingtone');
                          },
                        );
                      },
